 ...
 # list() -> confusion_matrix_list #
  ...
  for (f in sort(unique(dane$fold)) ) {
   ...
   #deepviz::plot_model(model)
    ...
    # valid_target %>%
    #   reverse_one_hot() -> valid_target_list[[f]]
...
} # evaluation all cv models on test set
...
naive_ensemble_prediction %>%
  bind_rows() %>%
  set_colnames(colnames(test_target)) %>%
  reverse_one_hot() %>%
  enframe(name = NULL, value = "predicted") %>%
  mutate(true = test_target %>% reverse_one_hot()) -> tmp

confusionMatrix(tmp$predicted, tmp$true) %>%
.$byClass %>% as.data.frame() %>% pull(`Balanced Accuracy`) -> Balanced_Accuracy
  confusionMatrix(tmp$predicted, tmp$true) %>% .$table %>%
    rbind( Balanced_Accuracy) %>%
    superheat(  
      X.text = round(as.matrix(.), 3), order.rows = order( rownames(.) ,decreasing = F),
      row.title = "Prediction",left.label.text.size =  4, bottom.label.text.size  = 4,
...
show_emb <- function(DATA, embeddings_index,  variable = "main", top_idf = 100, dims = 2, perplex = 10, theta = 0.1, do_pca= T, ...) {
  if(!top_idf - 1 > 3*perplex) stop("Perplexity too big")
  ggsci::pal_d3() -> paletka
  list() -> lista_emb
  DATA %>%
  ...
  tsne_out <- Rtsne(X = as.matrix(data_after_emb %>% select(starts_with("V"))),
                    dims = dims, perplexity = perplex, theta = theta, pca = do_pca)
  ...
  data_to_plot %<>% mutate(tf_idf = round(tf_idf, 5) )
  ...
  if(dims == 3){
    data_to_plot %>%
      plot_ly(. , x = ~ V1, y = ~V2, z = ~V3,
              ...
              # type = 'scatter',
              # mode = 'markers',
              #marker = list(size = ~tf_idf),
              size = ~  tf_idf,  #alpha = 0.8,
              colors = paletka(unique(tsne_df$category) %>% length()),
