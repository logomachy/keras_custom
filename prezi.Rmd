...
flagi <- flags(
  ...
  flag_numeric("first_bidirectional_drop", 0.05), 
  flag_numeric("first_bidirectional_rec_drop", 0.1), 
  
  
  flag_integer("second_bidirectional_units", 256), 
  flag_numeric("second_bidirectional_drop", 0.05), 
  flag_numeric("second_bidirectional_rec_drop", 0.1),
...
confusion_matrix_plot <- function(model, valid_X, valid_target, train_data) {
  ...
  confuse_a_cat %>%
    .$byClass %>% as.data.frame() %>% pull(`Balanced Accuracy`) -> B_Accuracy
  confuse_a_cat %>% .$table %>%
    rbind( B_Accuracy) %>%
    superheat(  
...
```{r echo=FALSE}
set.seed(1)
DATA %>% group_by(category) %>% sample_n(1) %>% select(name, category, text)
```
...
$$tf(slowo, artykul) = \frac{n_{slowa \ w \ artykule}}{\sum n_{ liczby\ wystapien\ wszystkich \ slow\ \ w \ artukule}} 
\\ idf(slowo) = ln(\frac{n_{artykulow \ w \ korpusie}}{n_{artykulow \ zawierajacych \ slowo}}) $$
...
```{r echo=F, fig.height=6, fig.width=6, warning=F}
...
```{r echo=F, fig.height=6, fig.width=6, warning=FALSE}
...
deepviz::plot_model(model_list[[f]])
```
...
```{r echo=F, fig.height=6, fig.width=8, warning=FALSE}
confusion_matrix_plot(model_list[[f]], valid_X, valid_target, train_data )
```
...
```{r echo=FALSE, fig.height=6, fig.width=8, warning=FALSE}
 confusion_matrix_plot(model_list[[f]], test_X, test_target, test_target)
```
